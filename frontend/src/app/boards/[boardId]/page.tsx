...
import { Trash2Icon, Plus, ArrowLeft, Sparkles } from "lucide-react";
    ...
    <div className="relative min-h-screen bg-[#080d0b] overflow-hidden font-[Inter,sans-serif]">
      ...
      <nav className="sticky top-0 z-50 flex items-center justify-between px-4 md:px-8 py-3 md:py-4 border-b border-white/6 bg-[#080d0b]/70 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3 text-xs text-white/40 shrink-0">
          <Button
            className="flex gap-2 hover:cursor-pointer transition-colors"
            size="sm"
            onClick={() => router.push("/taskoraai")}
          >
        ...
        {loading ? (
          ...
          <div className="flex gap-3 md:gap-4 overflow-x-auto pb-8 items-start snap-x snap-mandatory">
            ...
            {columns.map((column, ci) => {
              ...
              const activeTasks =
                column.task?.filter((t) => !isTaskExpired(t.due_date)) ?? [];
                          ...
                          <CardContent className="p-3 pb-3">
                            ...
                            {task.due_date && (
                              <div className="flex items-center gap-1 pt-2 border-t border-white/6">
                                <span
                                  className={`text-[10px] tracking-wide ${getDueDateColor(task.due_date)}`}
                                >
                                  {isTaskExpired(task.due_date)
                                    ? "⚠️ Overdue: "
                                    : "📅 "}
                                  {task.due_date}
                                </span>
